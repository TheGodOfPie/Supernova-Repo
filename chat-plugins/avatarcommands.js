...
var cmds = {
	...
	help: function (target, room, user) {
		...
		return this.sendReplyBox('<b>Custom Avatar commands</b><br>' +
			'(All commands require Global Driver (%) and above)<br><br>' +
	...
	set: function (target, room, user, connection, cmd) {
		...
		if (!this.can('lock') && isVip(user) && toId(targetUser) !== user.userid) return this.errorReply('Vip users can only set avatar for themselves.'); 
	...
	'delete': function (target, room, user, connection, cmd) {
		...
		if (!this.can('lock') && isVip(user) && toId(target) !== user.userid) return this.errorReply('Vip users can only remove their avatars.'); 
