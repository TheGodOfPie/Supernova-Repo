...
exports.commands = {
	poll: function (target, room, user) {
		...
		var pollOptions = '';
		var start = 0;
		pollOptions += '<div style="padding: 3px; background: rgba(204, 102, 255,0.5);"><br /><center>';
		while (start < Poll[room.id].optionList.length) {
			pollOptions += '<button name="send" value="/vote ' + Tools.escapeHTML(Poll[room.id].optionList[start]) + '" style="border-radius: 3px; box-shadow: 0px 1px 0px rgba(0, 0, 0, 0.08);" class="super-button">' + Tools.escapeHTML(Poll[room.id].optionList[start]) + '</button>&nbsp;';	
			start++;
		}
		pollOptions += '</center><br /></div>';
		Poll[room.id].display = '<div style="background: #993366; border-bottom: 1px solid #000; box-shadow: 0px 1px 1px rgba(124,68,101,0.35) inset; padding: 3px;"><center><font size="5" color="white">' + Tools.escapeHTML(Poll[room.id].question) + '</font><font size="1" color="#CCC" style="font-style: italic;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; - Poll started by ' + user.name + '</font></center></div>' + pollOptions;
	...
	endpoll: function (target, room, user) {
		...
		var results = '<div style="padding: 3px; background: rgba(204, 102, 255,0.5);"><br />';
		var len = data.length;
