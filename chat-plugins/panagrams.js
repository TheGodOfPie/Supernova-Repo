...
var Panagram = (function () {
	...
	Panagram.prototype.guess = function (user, guess) {
		...
		if (guess.species === this.answer.species) {
			giveTicket(user.userid);
			this.room.add('|html|<b><font color="' + color(user.userid) + '">' + user.name + '</font></b> guessed <b><font color=green>' + guess.species + '</font></b>, which was the correct answer! This user has also won 1 ticket!');
			this.end();
		} else {
			this.room.add('|html|<b><font color="' + color(user.userid) + '">' + user.name + '</font></b> guessed <b><font color=red>' + guess.species + '</font></b>, but was not the correct answer.');
...
exports.commands = {
	...
	panagram: function (target, room, user, connection, cmd) {
		...
		if (room.id !== 'casino') return this.sendReply('Panagrams can only be played in the Casino.');
		if (!target || isNaN(target)) return this.errorReply("Usage: /panagram [number of sessions]");
		if (target < 25) return this.errorReply("The minimum number of sessions you can have at a time is 25.");
		if (target > 25) return this.errorReply("The maximum number of sessions you can have at a time is 25.");
	...
	panagramend: function (target, room, user, connection, cmd) {
		if (!pGames[room.id]) return this.errorReply("There is no game of panagram going on in this room.");
		if (!this.can('roomban', null, room)) return this.sendReply("You must be ranked @ or higher to end a game of panagram in this room.");

		var skipCmd = ((cmd === 'panagramskip' || cmd === 'pskip') && pGames[room.id].sessions > 1);
		if (skipCmd) room.add('|html|The current session of panagram has been ended by ' + user.name + '. The answer was <b><font color="yellow">' + pGames[room.id].answer.species + '</font></b>.');
